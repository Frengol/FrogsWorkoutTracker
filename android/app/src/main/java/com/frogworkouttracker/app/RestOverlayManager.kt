...
class RestOverlayManager(
  ...
  private val onPressed: (String?, String?) -> Unit,
) {
  companion object {
    ...
    private const val ENDED_TEXT = "Encerrado, toque para voltar"
  }
  ...
  private var openButtonView: TextView? = null
  ...
  fun show(workoutId: String, sourceSetId: String?, endsAtMs: Long, isFinished: Boolean) {
  ...
  fun update(workoutId: String, sourceSetId: String?, endsAtMs: Long, isFinished: Boolean) {
    this.workoutId = workoutId
    this.sourceSetId = sourceSetId
    this.endsAtMs = endsAtMs
    this.isFinished = isFinished
    runOnMainThread {
      ...
      if (this.isFinished || remainingSeconds() <= 0) {
        stopTicker()
      } else {
        startTicker()
      }
      ...
      override fun onTouch(view: View, event: MotionEvent): Boolean {
        when (event.actionMasked) {
          ...
          MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
            if (event.actionMasked == MotionEvent.ACTION_UP && !hasMoved && isFinished) {
              onPressed(workoutId, sourceSetId)
            }
            return true
          }
  ...
  private fun updateOverlayContent() {
    ...
    openButtonView?.visibility = if (isFinished) View.GONE else View.VISIBLE
  }
