...
const AUTO_BACKUP_DIRECTORY_NAME = 'frog-auto-backup';
const AUTO_BACKUP_FILE_NAME = 'frog-backup-v1.json';
...
const AUTO_BACKUP_TABLES = BACKUP_TABLES;
...
const hasEssentialBackupTableData = (tables: BackupEnvelopeV1['tables']) =>
  BACKUP_TABLES.some((tableName) => Array.isArray(tables[tableName]) && tables[tableName].length > 0);

const parseBackupEnvelope = (content: string) => {
  const parsed = backupSchema.parse(JSON.parse(content));
  if (!hasEssentialBackupTableData(parsed.tables)) {
    throw new Error('Cópia de segurança sem dados essenciais.');
  }

  return parsed;
};

const parseBackupEnvelopeValue = (value: unknown): BackupEnvelopeV1 => {
...
const sanitizeBackupRow = (tableName: string, row: Record<string, unknown>) => {
  if (tableName === 'user_preferences') {
    const {
      default_workout_visibility: _removedVisibility,
      haptics_enabled: _removedHaptics,
      show_rpe: _removedRpe,
      show_previous_values: _removedPreviousValues,
      ...sanitized
    } = row;
    return sanitized;
  }
  ...
  if (tableName === 'exercises') {
    const { is_archived: _removed, ...sanitized } = row;
    return sanitized;
  }
...
const sanitizeBackupEnvelope = (envelope: BackupEnvelopeV1): BackupEnvelopeV1 => ({
  ...envelope,
  tables: Object.fromEntries(
    BACKUP_TABLES.map((tableName) => [
      tableName,
      (envelope.tables[tableName] ?? []).map((row) => sanitizeBackupRow(tableName, row)),
    ]),
  ),
});
...
const routineJsonRoutineSchema = z.object({
  id: z.string().optional(),
  name: z.string().min(1),
...
const routineJsonEnvelopeSchema = z.object({
  ...
  version: z.literal(1),
  exportedAt: z.string(),
  routine: routineJsonRoutineSchema,
});

const routinesJsonEnvelopeSchema = z.object({
  kind: z.literal('frog_routines'),
  version: z.literal(1),
  exportedAt: z.string(),
  routines: z.array(routineJsonRoutineSchema).default([]),
});
...
const workoutRowSchema = z.object({
  ...
  secondary_muscles_json: z.string().optional().default('[]'),
  equipment: z.string().optional().default('other'),
  modality: z.string().optional().default('strength'),
  instructions: z.string().optional().default('Exercicio importado localmente.'),
...
const getAutoBackupDirectory = () => new Directory(Paths.document, AUTO_BACKUP_DIRECTORY_NAME);

const getWritableAutoBackupDirectory = () => {
  const directory = getAutoBackupDirectory();
  if (!directory.exists) {
    directory.create({ idempotent: true, intermediates: true });
  }
  return directory;
...
const getAutoBackupFile = () => new File(getAutoBackupDirectory(), AUTO_BACKUP_FILE_NAME);

const writeAutoBackupFile = (content: string) => {
  const file = new File(getWritableAutoBackupDirectory(), AUTO_BACKUP_FILE_NAME);
...
type WorkoutCsvExportOptions = {
  workoutId?: string;
  workoutIds?: string[];
};

type ExportWorkoutsCsvOptions = {
  workoutIds?: string[];
};

const toWorkoutCsvRows = ({ workoutId, workoutIds }: WorkoutCsvExportOptions = {}): WorkoutCsvRow[] => {
  const selectedWorkoutIds =
    workoutIds === undefined
      ? null
      : Array.from(new Set(workoutIds.map((id) => id.trim()).filter((id) => id.length > 0)));

  if (selectedWorkoutIds?.length === 0) {
    return [];
  }

  const workoutIdClause = selectedWorkoutIds
    ? `AND w.id IN (${selectedWorkoutIds.map(() => '?').join(', ')})`
    : workoutId
      ? 'AND w.id = ?'
      : '';
  const params = selectedWorkoutIds ?? (workoutId ? [workoutId] : []);
  ...
  return database.getAllSync<WorkoutCsvRow>(
    `
      SELECT
        ...
        COALESCE(e.secondary_muscles_json, '[]') AS secondary_muscles_json,
...
export const exportWorkoutsCsv = async (options: ExportWorkoutsCsvOptions = {}) => {
  const isSelectionExport = options.workoutIds !== undefined;
  const rows = toWorkoutCsvRows({ workoutIds: options.workoutIds });
  if (isSelectionExport && rows.length === 0) {
    throw new Error('Nenhum treino encontrado para exportar.');
  }
...
const toRoutineJsonRoutine = (routineId: string) => {
...
const toRoutineJsonEnvelope = (routineId: string) => {
  const routine = toRoutineJsonRoutine(routineId);
  if (!routine) {
    return null;
  }
...
export const exportRoutinesJson = async ({ routineIds }: { routineIds?: string[] } = {}) => {
  const exportRoutineIds = routineIds?.length
    ? [...new Set(routineIds)]
    : database.getAllSync<{ id: string }>(
        `
          SELECT id
          FROM routines
  ...
  const routines = exportRoutineIds
    .map((routineId) => toRoutineJsonRoutine(routineId))
    .filter((routine): routine is NonNullable<ReturnType<typeof toRoutineJsonRoutine>> => Boolean(routine));

  if (routines.length === 0) {
    throw new Error('Nenhuma rotina encontrada para exportar.');
  }

  const envelope = {
    kind: 'frog_routines' as const,
    version: 1 as const,
    exportedAt: nowIso(),
    routines,
  };
  const file = writeTextFile(`frog-routines-${new Date().toISOString().slice(0, 10)}.json`, JSON.stringify(envelope, null, 2));
  writeAuditLog('data_export', 'routines_json', 'exported', {
    routines: routines.length,
    uri: file.uri,
  });
  return shareFile(file, 'application/json', 'Compartilhar rotinas');
};
...
const createBackupEnvelope = (
  tableNames: readonly string[] = BACKUP_TABLES,
  exportedAt = nowIso(),
): BackupEnvelopeV1 => ({
    version: 1,
    exportedAt,
    deviceId: getDeviceId(),
    tables: Object.fromEntries(tableNames.map((tableName) => [tableName, getTableRows(tableName)])),
  });

const getAutoBackupFileSize = () => {
  const file = getAutoBackupFile();
  return file.exists ? (file.info().size ?? 0) : 0;
};

const readAutoBackupStatus = (): AutoBackupStatus => {
  const preferences = getUserPreferences();

  return {
    enabled: preferences?.auto_backup_enabled === 1,
    lastUpdatedAt: preferences?.auto_backup_last_exported_at ?? null,
    fileSizeBytes: getAutoBackupFileSize(),
  };
};

const updateAutoBackupPreference = (enabled: boolean, lastUpdatedAt: string | null) => {
  const preferences = getUserPreferences();
  if (!preferences) {
    throw new Error('Preferências locais não inicializadas.');
  }
  ...
  database.runSync(
    `
      ...
      SET auto_backup_enabled = ?, auto_backup_last_exported_at = ?, updated_at = ?
      WHERE id = ?
    `,
    enabled ? 1 : 0,
    lastUpdatedAt,
...
const writeAutoBackupSnapshotSync = () => {
  const exportedAt = nowIso();
  const backup = createBackupEnvelope(AUTO_BACKUP_TABLES, exportedAt);
  const file = writeAutoBackupFile(JSON.stringify(backup, null, 2));
  updateAutoBackupPreference(true, exportedAt);
  writeAuditLog('data_export', 'android_auto_backup', 'snapshot_written', {
    uri: file.uri,
    tables: AUTO_BACKUP_TABLES.length,
  });
  return {
    enabled: true,
    lastUpdatedAt: exportedAt,
    fileSizeBytes: file.info().size ?? 0,
  };
};

export const getAutoBackupStatus = () => readAutoBackupStatus();

export const writeAutoBackupSnapshot = async () => writeAutoBackupSnapshotSync();

export const syncAutoBackupSnapshotIfEnabled = async () => {
  const status = readAutoBackupStatus();
  if (!status.enabled) {
    return status;
  }

  return writeAutoBackupSnapshotSync();
};

export const setAutoBackupEnabled = async (enabled: boolean) => {
  if (!enabled) {
    deleteAutoBackupPayload();
    updateAutoBackupPreference(false, null);
    writeAuditLog('data_export', 'android_auto_backup', 'disabled', {});
    return {
      enabled: false,
      lastUpdatedAt: null,
      fileSizeBytes: 0,
  ...
  const status = writeAutoBackupSnapshotSync();
  writeAuditLog('data_export', 'android_auto_backup', 'enabled', {
...
export const exportBackupJson = async () => {
  const backup = createBackupEnvelope(BACKUP_TABLES);
...
const commonUtf8MojibakePattern = /(?:Ã[\u0080-\u00bf]|Â[\u0080-\u00bf])/;
const commonUtf8MojibakePatternGlobal = /(?:Ã[\u0080-\u00bf]|Â[\u0080-\u00bf])/g;

const stripUtf8Bom = (value: string) => (value.charCodeAt(0) === 0xfeff ? value.slice(1) : value);

const isUtf8ContinuationByte = (byte: number | undefined): byte is number =>
  byte != null && (byte & 0xc0) === 0x80;

const stringFromCodePoint = (codePoint: number) => {
  if (codePoint <= 0xffff) {
...
const appendCodePointAsUtf8 = (codePoint: number, target: number[]) => {
  if (codePoint <= 0x7f) {
  ...
  if (codePoint <= 0xffff) {
    target.push(0xe0 | (codePoint >> 12), 0x80 | ((codePoint >> 6) & 0x3f), 0x80 | (codePoint & 0x3f));
    return;
  }
  ...
  while (index < bytes.length) {
    ...
    if (first >= 0xc2 && first <= 0xdf) {
      ...
      if (isUtf8ContinuationByte(second)) {
        index += 1;
        chars.push(stringFromCodePoint(((first & 0x1f) << 6) | (second & 0x3f)));
        continue;
      }
    }
    ...
    if (first >= 0xe0 && first <= 0xef) {
      ...
      if (validSecond && isUtf8ContinuationByte(third)) {
        index += 2;
        chars.push(stringFromCodePoint(((first & 0x0f) << 12) | ((second & 0x3f) << 6) | (third & 0x3f)));
        continue;
      }
    }
    ...
    chars.push(utf8ReplacementChar);
  }

  return chars.join('');
};

const getUtf8MojibakeMarkerCount = (value: string) => value.match(commonUtf8MojibakePatternGlobal)?.length ?? 0;

const encodeLegacyTextForMojibakeRepair = (value: string) => {
  const bytes: number[] = [];
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (codeUnit >= 0xd800 && codeUnit <= 0xdbff && index + 1 < value.length) {
      ...
      if (nextCodeUnit >= 0xdc00 && nextCodeUnit <= 0xdfff) {
        appendCodePointAsUtf8(0x10000 + ((codeUnit - 0xd800) << 10) + (nextCodeUnit - 0xdc00), bytes);
        index += 1;
        continue;
      }
    }
    ...
    appendCodePointAsUtf8(codeUnit, bytes);
  }
...
const repairUtf8Mojibake = (value: string) => {
  const normalizedValue = stripUtf8Bom(value);
  if (!commonUtf8MojibakePattern.test(normalizedValue)) {
    return normalizedValue;
  }

  const repaired = stripUtf8Bom(decodeUtf8Bytes(encodeLegacyTextForMojibakeRepair(normalizedValue)));
  return getUtf8MojibakeMarkerCount(repaired) < getUtf8MojibakeMarkerCount(normalizedValue) ? repaired : normalizedValue;
};

const decodeUtf8Text = (bytes: Uint8Array) => repairUtf8Mojibake(decodeUtf8Bytes(bytes));

const readTextFileContent = async (uri: string) => {
  const file = new File(uri);

  try {
    return decodeUtf8Text(await file.bytes());
  } catch {
    return repairUtf8Mojibake(await file.text());
  }
};

const isContentUri = (uri: string) => uri.trim().toLowerCase().startsWith('content://');

const sanitizeExternalImportFileName = (fileName: string) =>
  fileName
    .trim()
    .replace(/[^a-zA-Z0-9._-]+/g, '-')
    .replace(/^-+|-+$/g, '') || 'arquivo-importado';

const getExternalImportCacheUri = (fileName: string) => {
  if (!LegacyFileSystem.cacheDirectory) {
    throw new Error('Cache directory unavailable');
  }

  const cacheDirectory = LegacyFileSystem.cacheDirectory.endsWith('/')
    ? LegacyFileSystem.cacheDirectory
    : `${LegacyFileSystem.cacheDirectory}/`;
  return `${cacheDirectory}frogs-external-import-${Date.now()}-${sanitizeExternalImportFileName(fileName)}`;
};

const readExternalFileContent = async (uri: string, fileName: string) => {
  if (!isContentUri(uri)) {
    return readTextFileContent(uri);
  }

  const cachedUri = getExternalImportCacheUri(fileName);
  try {
    await LegacyFileSystem.copyAsync({ from: uri, to: cachedUri });
    return await readTextFileContent(cachedUri);
  } finally {
...
const readPickedFile = async (acceptedTypes: string | string[]) => {
  ...
  return {
    fileName: asset.name,
    content: await readTextFileContent(asset.uri),
  };
};

type RoutineJsonEnvelope = z.infer<typeof routineJsonEnvelopeSchema>;
type RoutineJsonRoutine = z.infer<typeof routineJsonRoutineSchema>;
...
const isKnownMuscleGroup = (value: string): value is MuscleGroup => muscleGroups.includes(value as MuscleGroup);
const isKnownEquipment = (value: string): value is Equipment => equipmentOptions.includes(value as Equipment);
const isKnownModality = (value: string): value is ExerciseModality => modalityOptions.includes(value as ExerciseModality);

const normalizeImportedMuscleGroup = (value: string | null | undefined, fallback: MuscleGroup = 'full_body'): MuscleGroup =>
  value && isKnownMuscleGroup(value) ? value : fallback;

const normalizeImportedEquipment = (value: string | null | undefined): Equipment =>
  value && isKnownEquipment(value) ? value : 'other';

const normalizeImportedModality = (value: string | null | undefined): ExerciseModality =>
  value && isKnownModality(value) ? value : 'strength';

const normalizeImportedSecondaryMuscles = (
  values: readonly string[],
  primaryMuscle: MuscleGroup,
): MuscleGroup[] =>
  values
    .filter(isKnownMuscleGroup)
    .filter((muscleGroup) => muscleGroup !== primaryMuscle)
    .slice(0, 4);

type ImportedExercisePlaceholderDraft = {
...
type ImportedExerciseComparisonResult = {
  differenceCount: number;
  matchedExerciseId: string | null;
  matchedExerciseIsCustom: boolean | null;
};
...
const normalizeComparisonText = (value: string | null | undefined) =>
  normalizeRoutineImportText(value)
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/\s+/g, ' ');

const normalizeComparisonMuscles = (values: readonly string[]) =>
  [...new Set(values.filter(isKnownMuscleGroup))].sort();

const parseComparisonSecondaryMuscles = (value: string | null | undefined) => {
  try {
    const parsed = value ? JSON.parse(value) : [];
    return Array.isArray(parsed) ? normalizeComparisonMuscles(parsed.map(String)) : [];
  } catch {
    return [];
  }
};

const countImportedExerciseDifferences = (
  draft: ImportedExercisePlaceholderDraft,
  existing: ExerciseComparisonRow,
) => {
  let differenceCount = 0;

  if (normalizeComparisonText(draft.name) !== normalizeComparisonText(existing.name)) {
    differenceCount += 1;
  }

  if (draft.muscleGroup !== normalizeImportedMuscleGroup(existing.muscle_group)) {
    differenceCount += 1;
  }

  const draftSecondary = normalizeComparisonMuscles(draft.secondaryMuscles);
  const existingSecondary = parseComparisonSecondaryMuscles(existing.secondary_muscles_json);
  if (JSON.stringify(draftSecondary) !== JSON.stringify(existingSecondary)) {
    differenceCount += 1;
  }

  if (draft.equipment !== normalizeImportedEquipment(existing.equipment)) {
    differenceCount += 1;
  }

  if (draft.modality !== normalizeImportedModality(existing.modality)) {
    differenceCount += 1;
  }

  if (normalizeComparisonText(draft.instructions) !== normalizeComparisonText(existing.instructions)) {
    differenceCount += 1;
  }
...
const getExistingExerciseComparisonRows = () =>
  database.getAllSync<ExerciseComparisonRow>(
    `
      SELECT id, name, muscle_group, secondary_muscles_json, equipment, modality, is_custom, instructions
      FROM exercises
      WHERE deleted_at IS NULL
    `,
  );

const compareImportedExerciseWithExistingBase = (
  draft: ImportedExercisePlaceholderDraft,
): ImportedExerciseComparisonResult => {
  const normalizedName = normalizeComparisonText(draft.name);
  const candidates = getExistingExerciseComparisonRows().filter(
    (exercise) => normalizeComparisonText(exercise.name) === normalizedName,
  );
  ...
  if (candidates.length === 0) {
    return {
      ...
      matchedExerciseIsCustom: null,
    };
  }

  const [bestMatch] = candidates
    .map((candidate) => ({
      differenceCount: countImportedExerciseDifferences(draft, candidate),
      matchedExerciseId: candidate.id,
      matchedExerciseIsCustom: Number(candidate.is_custom) === 1,
    }))
...
const shouldAutoMatchImportedStandardExercise = (
  importedExerciseIsStandard: boolean,
  comparison: ImportedExerciseComparisonResult,
) =>
  importedExerciseIsStandard &&
  comparison.differenceCount === 0 &&
  Boolean(comparison.matchedExerciseId) &&
  comparison.matchedExerciseIsCustom === false;

const createImportedExercisePlaceholder = (draft: ImportedExercisePlaceholderDraft) => {
  const base = createEntityBase();
  const exerciseName = normalizeRoutineImportText(draft.name) || 'Exercício importado';
  const preferredId = normalizeRoutineImportText(draft.preferredId);
  const exerciseId = preferredId && !findExerciseById(preferredId) ? preferredId : createId();
  const slug = buildUniqueImportedExerciseSlug(exerciseName, draft.preferredSlug);
  ...
  database.runSync(
    ...
    exerciseName,
    draft.muscleGroup,
    JSON.stringify(draft.secondaryMuscles),
    draft.equipment,
    draft.modality,
    1,
    normalizeRoutineImportText(draft.instructions),
  );

  return exerciseId;
};

const buildImportedRoutineExerciseDraft = (exercise: RoutineJsonExercise): ImportedExercisePlaceholderDraft => {
  const muscleGroup = normalizeImportedMuscleGroup(exercise.exercise.muscleGroup);

  return {
    preferredId: exercise.exerciseId ?? exercise.exercise.id,
    preferredSlug: exercise.exercise.slug,
    name: exercise.exercise.name,
    muscleGroup,
    secondaryMuscles: normalizeImportedSecondaryMuscles(exercise.exercise.secondaryMuscles, muscleGroup),
    equipment: normalizeImportedEquipment(exercise.exercise.equipment),
    modality: normalizeImportedModality(exercise.exercise.modality),
    instructions: normalizeRoutineImportText(exercise.exercise.instructions ?? ''),
  };
};

const backupRowString = (row: Record<string, unknown>, key: string) => {
  const value = row[key];
  return typeof value === 'string' ? value : value == null ? '' : String(value);
};

const backupRowNumber = (row: Record<string, unknown>, key: string) => {
  const value = row[key];
  const parsed = typeof value === 'number' ? value : Number(value);
  return Number.isFinite(parsed) ? parsed : null;
};

const buildImportedBackupExerciseDraft = (row: Record<string, unknown>): ImportedExercisePlaceholderDraft => {
  const muscleGroup = normalizeImportedMuscleGroup(backupRowString(row, 'muscle_group'));

  return {
    preferredId: backupRowString(row, 'id'),
    preferredSlug: backupRowString(row, 'slug'),
    name: normalizeRoutineImportText(backupRowString(row, 'name')) || 'Exercício importado',
    muscleGroup,
    secondaryMuscles: normalizeImportedSecondaryMuscles(
      parseJsonStringArray(backupRowString(row, 'secondary_muscles_json')),
      muscleGroup,
    ),
    equipment: normalizeImportedEquipment(backupRowString(row, 'equipment')),
    modality: normalizeImportedModality(backupRowString(row, 'modality')),
    instructions: normalizeRoutineImportText(backupRowString(row, 'instructions')),
  };
};
...
const parseRoutineJsonImportRoutines = (content: string): RoutineJsonRoutine[] => {
  const parsedJson = JSON.parse(content) as { kind?: unknown };

  if (parsedJson.kind === 'frog_routines') {
    const parsedBatch = routinesJsonEnvelopeSchema.parse(parsedJson);
    if (parsedBatch.routines.length === 0) {
      throw new Error('empty routines batch');
    }
    return parsedBatch.routines;
  }

  const parsedSingle = routineJsonEnvelopeSchema.parse(parsedJson);
  return [parsedSingle.routine];
};
...
const importRoutineJsonContent = (content: string, fileName: string): ImportJobResult => {
  ...
  try {
    const routinesToImport = parseRoutineJsonImportRoutines(content);
    ...
    const exerciseResolutionByReviewKey = new Map<string, { placeholderExerciseId: string; resolvedExerciseId: string }>();
    const auditItems: { routineId: string; originalRoutineId: string | null; exercises: number }[] = [];
    let exercisesCount = 0;

    runInTransaction(() => {
      routinesToImport.forEach((routine) => {
        const routineBase = createEntityBase();
        const folder = findOrCreateImportedRoutineFolder(routine.folderName);
        ...
        const exercises = routine.exercises.map((exercise, index) => {
          ...
          let resolution = exerciseResolutionByReviewKey.get(reviewKey);
          if (!resolution) {
            const draft = buildImportedRoutineExerciseDraft(exercise);
            const comparison = compareImportedExerciseWithExistingBase(draft);
            const placeholderExerciseId = createImportedExercisePlaceholder(draft);
            const isMatched = comparison.differenceCount === 0 && comparison.matchedExerciseId;
            const status = shouldAutoMatchImportedStandardExercise(exercise.exercise.isCustom === false, comparison)
              ? 'auto_matched'
              : isMatched
                ? 'matched'
                : 'pending';
            resolution = {
              ...
              resolvedExerciseId: isMatched ? comparison.matchedExerciseId! : placeholderExerciseId,
            };
            exerciseResolutionByReviewKey.set(reviewKey, resolution);
            ...
            reviewGroupsByKey.set(reviewKey, {
              ...
              matchedExerciseId: comparison.matchedExerciseId,
              resolvedExerciseId: isMatched ? comparison.matchedExerciseId : null,
            });
          }

          return {
            ...exercise,
            exerciseId: resolution.resolvedExerciseId,
            reviewKey,
            sortOrder: index,
          };
        });
        const routineHasReview = exercises.some((exercise) => Boolean(exercise.reviewKey));
        ...
        database.runSync(
          ...
          folder.id,
          routine.name.trim(),
          normalizeRoutineImportText(routine.description),
          'copied',
          Math.max(20, exercises.length * 9),
          routineHasReview ? 1 : 0,
        );
        ...
        importedRoutineIds.push(routineBase.id);
        exercisesCount += routine.exercises.length;
        auditItems.push({
          routineId: routineBase.id,
          originalRoutineId: routine.id ?? null,
          exercises: routine.exercises.length,
        });
      });
    });

    const insertedCount = importedRoutineIds.length;
    ...
    const importJobId = recordImportJob(result, checksum, fileName, reviewSummary ?? {
      insertedCount,
      skippedCount: 0,
      routineId: importedRoutineIds[0] ?? null,
      routineIds: importedRoutineIds,
      exercisesCount,
    });
    if (reviewSummary) {
    ...
    auditItems.forEach((item) => {
      writeAuditLog('routine', item.routineId, 'imported_json', {
        fileName,
        originalRoutineId: item.originalRoutineId,
...
const insertWorkoutCsvRows = (
  ...
  fileName: string,
  checksum: string,
  options: { reviewExercises?: boolean } = {},
): ImportJobResult => {
  const result: ImportJobResult = {
  ...
  const placeholderExerciseIds: string[] = [];
  const reviewGroupsByKey = new Map<string, ImportReviewGroupSummary>();
  const exerciseResolutionByKey = new Map<string, string>();
  ...
  runInTransaction(() => {
    rows.forEach((row) => {
      ...
      if (!exerciseAlreadyInserted) {
        ...
        if (options.reviewExercises) {
          let placeholderExerciseId = exerciseResolutionByKey.get(importedExerciseKey);

          if (!placeholderExerciseId) {
            const muscleGroup = normalizeImportedMuscleGroup(parsed.muscle_group);
            const draft: ImportedExercisePlaceholderDraft = {
              preferredId: parsed.exercise_id,
              name: parsed.exercise_name,
              muscleGroup,
              secondaryMuscles: normalizeImportedSecondaryMuscles(
                parseJsonStringArray(parsed.secondary_muscles_json),
                muscleGroup,
              ),
              equipment: normalizeImportedEquipment(parsed.equipment),
              modality: normalizeImportedModality(parsed.modality),
              instructions: normalizeRoutineImportText(parsed.instructions),
            };
            const comparison = compareImportedExerciseWithExistingBase(draft);
            const isMatched = comparison.differenceCount === 0 && comparison.matchedExerciseId;
            placeholderExerciseId = createImportedExercisePlaceholder(draft);
            placeholderExerciseIds.push(placeholderExerciseId);
            ...
            exerciseResolutionByKey.set(
              importedExerciseKey,
              isMatched ? comparison.matchedExerciseId! : placeholderExerciseId,
            );
          }

          resolvedExerciseId = exerciseResolutionByKey.get(importedExerciseKey) ?? placeholderExerciseId;
        } else {
  ...
  const reviewSummary: ImportReviewSummary | undefined = options.reviewExercises
    ? {
        insertedCount: result.insertedCount,
        skippedCount: result.skippedCount,
...
type ReviewableImportSourceType = Extract<
  ImportSourceType,
  'hevy_csv' | 'frog_workouts_csv' | 'frog_routine_json' | 'frog_backup_json'
>;
...
const normalizeImportReviewStatus = (status: unknown): ImportReviewGroupSummary['status'] =>
  status === 'matched' || status === 'auto_matched' || status === 'replaced' || status === 'edited'
    ? status
    : 'pending';

const normalizeImportReviewGroup = (group: Partial<ImportReviewGroupSummary>): ImportReviewGroupSummary => {
  const workoutExerciseIds = Array.isArray(group.workoutExerciseIds) ? group.workoutExerciseIds.map(String) : [];
  const routineExerciseIds = Array.isArray(group.routineExerciseIds) ? group.routineExerciseIds.map(String) : [];
  const parsedDifferenceCount = Number(group.differenceCount);
  ...
  return {
    ...
    workoutExerciseIds,
    routineExerciseIds,
    status: normalizeImportReviewStatus(group.status),
    differenceCount: Number.isFinite(parsedDifferenceCount)
      ? parsedDifferenceCount
      : workoutExerciseIds.length + routineExerciseIds.length,
    matchedExerciseId: group.matchedExerciseId ? String(group.matchedExerciseId) : null,
    resolvedExerciseId: group.resolvedExerciseId ? String(group.resolvedExerciseId) : null,
  };
};

const normalizeBackupRestoreReviewSummary = (value: unknown): ImportReviewSummary['backupRestore'] => {
  ...
  const raw = value as {
    envelope?: unknown;
    exerciseIdsByGroupKey?: unknown;
  };
  const exerciseIdsByGroupKey =
    raw.exerciseIdsByGroupKey && typeof raw.exerciseIdsByGroupKey === 'object'
      ? Object.fromEntries(
          Object.entries(raw.exerciseIdsByGroupKey as Record<string, unknown>).map(([key, ids]) => [
            key,
  ...
  return {
    envelope: parseBackupEnvelopeValue(raw.envelope),
...
const parseImportReviewSummary = (summaryJson: string | null | undefined): ImportReviewSummary => {
  ...
  return {
    ...
    exerciseGroups: Array.isArray(raw.exerciseGroups)
      ...
      : [],
    backupRestore: normalizeBackupRestoreReviewSummary(raw.backupRestore),
  };
};
...
export const saveImportReview = (
  ...
  if (summary.backupRestore) {
    const resolvedEnvelope = resolveReviewedBackupEnvelope(summary);
    return applyBackupRestoreEnvelope(resolvedEnvelope, row.file_name);
  }

  runInTransaction(() => {
    const replacedPlaceholderIds = summary.exerciseGroups
      .filter((group) => group.status === 'replaced' || group.status === 'matched' || group.status === 'auto_matched')
...
const insertHevyCsvRows = (rows: Record<string, string>[], fileName: string, checksum: string): ImportJobResult => {
  ...
  runInTransaction(() => {
    ...
    rows.forEach((row, index) => {
      ...
      let exerciseId = exerciseResolutionByKey.get(importedExerciseKey);
      ...
      if (!exerciseId) {
        const draft: ImportedExercisePlaceholderDraft = {
          ...
          equipment: 'other',
          modality: 'strength',
          instructions: 'Exercicio importado localmente.',
        };
        const comparison = compareImportedExerciseWithExistingBase(draft);
        const isMatched = comparison.differenceCount === 0 && comparison.matchedExerciseId;
        const placeholderExerciseId = createImportedExercisePlaceholder(draft);
        exerciseId = isMatched ? comparison.matchedExerciseId! : placeholderExerciseId;
        placeholderExerciseIds.push(placeholderExerciseId);
        reviewGroupsByKey.set(importedExerciseKey, {
          key: importedExerciseKey,
        ...
        exerciseResolutionByKey.set(importedExerciseKey, exerciseId);
      }
...
const externalFileUnsupportedImportError =
...
const createUnsupportedExternalImportResult = (fileName: string): ImportJobResult => ({
  sourceType: 'frog_workouts_csv',
  fileName,
  status: 'failed',
  insertedCount: 0,
  skippedCount: 0,
  errors: [externalFileUnsupportedImportError],
});

const inferImportFileNameFromUri = (uri: string) => {
  try {
    ...
    return lastSegment ? decodeURIComponent(lastSegment.split('?')[0]) : null;
  }
};

const parseExternalJsonContent = (content: string) => {
  try {
...
const isExternalRoutineJsonValue = (value: unknown) =>
  typeof value === 'object' &&
  value !== null &&
  ('kind' in value) &&
  ((value as { kind?: unknown }).kind === 'frog_routine' || (value as { kind?: unknown }).kind === 'frog_routines');

const isExternalBackupJsonValue = (value: unknown) => {
  const parsed = backupSchema.safeParse(value);
  return parsed.success && hasEssentialBackupTableData(parsed.data.tables);
};

const importExternalFileContent = (content: string, fileName: string): ImportJobResult => {
  const trimmedContent = content.trim();
  const lowerFileName = fileName.toLowerCase();
  const shouldTreatAsJson = lowerFileName.endsWith('.json') || trimmedContent.startsWith('{');

  if (shouldTreatAsJson) {
    const parsedJson = parseExternalJsonContent(content);
    if (isExternalRoutineJsonValue(parsedJson)) {
      return importRoutineJsonContent(content, fileName);
    }

    if (isExternalBackupJsonValue(parsedJson)) {
      return restoreBackupContent(content, fileName);
    }

    return createUnsupportedExternalImportResult(fileName);
  }

  try {
    const { headers } = parseCsv(content);
    const detectedKind = detectCsvImportKind(headers);
    if (detectedKind === 'frog_workouts' || detectedKind === 'hevy_workouts') {
      return importCsvContent(content, fileName, {
        workoutOnly: true,
        reviewWorkoutExercises: true,
      });
    }
  } catch {
    return createUnsupportedExternalImportResult(fileName);
...
const importCsvContent = (
  content: string,
  fileName: string,
  options: { workoutOnly?: boolean; reviewWorkoutExercises?: boolean } = {},
): ImportJobResult => {
  const checksum = simpleChecksum(content);
  ...
  if (detectedKind === 'frog_workouts') {
    return insertWorkoutCsvRows(rows, fileName, checksum, {
      reviewExercises: options.reviewWorkoutExercises ?? true,
    });
  }
...
export const importExternalDataFile = async ({
  uri,
  fileName,
}: {
  uri: string;
  ...
  const resolvedFileName = fileName?.trim() || inferImportFileNameFromUri(uri) || 'arquivo-importado';
  let content: string;

  try {
    content = await readExternalFileContent(uri, resolvedFileName);
  } catch {
    return {
      sourceType: 'frog_workouts_csv',
      ...
      insertedCount: 0,
      skippedCount: 0,
      errors: ['Não foi possível ler o arquivo selecionado.'],
    } satisfies ImportJobResult;
  }

  try {
    return importExternalFileContent(content, resolvedFileName);
  } catch {
    return createUnsupportedExternalImportResult(resolvedFileName);
  }
};

const countBackupRows = (envelope: BackupEnvelopeV1) =>
  BACKUP_TABLES.reduce((count, tableName) => count + (envelope.tables[tableName]?.length ?? 0), 0);

const hasBackupExercises = (envelope: BackupEnvelopeV1) => (envelope.tables.exercises ?? []).length > 0;

const getRawExerciseRowById = (exerciseId: string) =>
  database.getFirstSync<Record<string, unknown>>('SELECT * FROM exercises WHERE id = ? LIMIT 1', exerciseId);

const createBackupRestoreReview = (
  envelope: BackupEnvelopeV1,
  fileName: string,
  checksum: string,
): ImportJobResult => {
  const sanitizedEnvelope = sanitizeBackupEnvelope(envelope);
  const exerciseRows = sanitizedEnvelope.tables.exercises ?? [];
  const placeholderExerciseIds: string[] = [];
  const reviewGroupsByKey = new Map<string, ImportReviewGroupSummary>();
  const exerciseIdsByGroupKey: Record<string, string[]> = {};

  runInTransaction(() => {
    exerciseRows.forEach((row) => {
      const draft = buildImportedBackupExerciseDraft(row);
      const importedName = draft.name;
      const reviewKey = importReviewGroupKey(importedName);
      const originalExerciseId = backupRowString(row, 'id');

      if (originalExerciseId) {
        exerciseIdsByGroupKey[reviewKey] = [...(exerciseIdsByGroupKey[reviewKey] ?? []), originalExerciseId];
      }

      if (reviewGroupsByKey.has(reviewKey)) {
        return;
      }
      ...
      const placeholderExerciseId = createImportedExercisePlaceholder(draft);
      const isMatched = comparison.differenceCount === 0 && comparison.matchedExerciseId;
      const status = shouldAutoMatchImportedStandardExercise(backupRowNumber(row, 'is_custom') === 0, comparison)
        ? 'auto_matched'
        : isMatched
          ? 'matched'
          : 'pending';
  ...
  const result: ImportJobResult = {
    ...
    insertedCount: countBackupRows(sanitizedEnvelope),
    skippedCount: 0,
    errors: [],
  };
  const importJobId = recordImportJob(result, checksum, fileName, {
    insertedCount: result.insertedCount,
    skippedCount: 0,
    workoutIds: [],
    routineIds: [],
...
const mergeEditedBackupExerciseRow = (
  originalRow: Record<string, unknown>,
  placeholderRow: Record<string, unknown>,
) => ({
  ...originalRow,
  slug: backupRowString(placeholderRow, 'slug') || backupRowString(originalRow, 'slug'),
  name: backupRowString(placeholderRow, 'name') || backupRowString(originalRow, 'name'),
  muscle_group: backupRowString(placeholderRow, 'muscle_group') || backupRowString(originalRow, 'muscle_group'),
  secondary_muscles_json:
    backupRowString(placeholderRow, 'secondary_muscles_json') || backupRowString(originalRow, 'secondary_muscles_json') || '[]',
  equipment: backupRowString(placeholderRow, 'equipment') || backupRowString(originalRow, 'equipment') || 'other',
  modality: backupRowString(placeholderRow, 'modality') || backupRowString(originalRow, 'modality') || 'strength',
  is_custom: placeholderRow.is_custom ?? originalRow.is_custom ?? 1,
  instructions: backupRowString(placeholderRow, 'instructions') || backupRowString(originalRow, 'instructions'),
  updated_at: backupRowString(placeholderRow, 'updated_at') || backupRowString(originalRow, 'updated_at') || nowIso(),
});

const remapExerciseReferenceRows = (
  rows: Record<string, unknown>[] | undefined,
  exerciseIdResolution: Map<string, string>,
) =>
  (rows ?? []).map((row) => {
    const exerciseId = backupRowString(row, 'exercise_id');
    const resolvedExerciseId = exerciseIdResolution.get(exerciseId);
    return resolvedExerciseId ? { ...row, exercise_id: resolvedExerciseId } : row;
  });

const resolveReviewedBackupEnvelope = (summary: ImportReviewSummary): BackupEnvelopeV1 => {
  const backupRestore = summary.backupRestore;
  if (!backupRestore) {
    throw new Error('Cópia de segurança pendente não encontrada.');
  }

  const envelope = sanitizeBackupEnvelope(backupRestore.envelope);
  const backupExerciseRows = envelope.tables.exercises ?? [];
  const backupExerciseRowsById = new Map<string, Record<string, unknown>>();
  backupExerciseRows.forEach((row) => {
    const exerciseId = backupRowString(row, 'id');
    if (exerciseId) {
      backupExerciseRowsById.set(exerciseId, row);
    }
  });
  const exerciseIdResolution = new Map<string, string>();
  const editedRowsByOriginalId = new Map<string, Record<string, unknown>>();
  const replacementRowsById = new Map<string, Record<string, unknown>>();

  summary.exerciseGroups.forEach((group) => {
    const originalExerciseIds = backupRestore.exerciseIdsByGroupKey[group.key] ?? [];

    if (group.status === 'replaced' || group.status === 'matched' || group.status === 'auto_matched') {
      const resolvedExerciseId = group.resolvedExerciseId;
      if (!resolvedExerciseId) {
        throw new Error('Exercício escolhido para a restauração não foi encontrado.');
      }

      const replacementRow = backupExerciseRowsById.get(resolvedExerciseId) ?? getRawExerciseRowById(resolvedExerciseId);
      if (!replacementRow) {
        throw new Error('Exercício escolhido para a restauração não foi encontrado.');
      }

      const sanitizedReplacementRow = sanitizeBackupRow('exercises', replacementRow);
      replacementRowsById.set(backupRowString(sanitizedReplacementRow, 'id'), sanitizedReplacementRow);
      originalExerciseIds.forEach((exerciseId) => {
        exerciseIdResolution.set(exerciseId, resolvedExerciseId);
      });
      return;
    }

    if (group.status === 'edited') {
      const placeholderRow = getRawExerciseRowById(group.placeholderExerciseId);
      if (!placeholderRow) {
        throw new Error('Exercício revisado não encontrado.');
      }

      originalExerciseIds.forEach((exerciseId) => {
        const originalRow = backupExerciseRowsById.get(exerciseId);
        if (!originalRow) {
          return;
        }

        exerciseIdResolution.set(exerciseId, exerciseId);
        editedRowsByOriginalId.set(
          exerciseId,
          sanitizeBackupRow('exercises', mergeEditedBackupExerciseRow(originalRow, placeholderRow)),
        );
      });
      return;
    }
  ...
  const finalExerciseRows = backupExerciseRows.filter((row) => {
    const exerciseId = backupRowString(row, 'id');
    const resolvedExerciseId = exerciseIdResolution.get(exerciseId);
    return !resolvedExerciseId || resolvedExerciseId === exerciseId;
  }).map((row) => editedRowsByOriginalId.get(backupRowString(row, 'id')) ?? row);
  const existingFinalExerciseIds = new Set(finalExerciseRows.map((row) => backupRowString(row, 'id')));
  replacementRowsById.forEach((row, exerciseId) => {
    if (!existingFinalExerciseIds.has(exerciseId)) {
      finalExerciseRows.push(row);
    }
  });
  ...
  return {
    ...
    tables: {
      ...envelope.tables,
      exercises: finalExerciseRows,
      workout_exercises: remapExerciseReferenceRows(envelope.tables.workout_exercises, exerciseIdResolution),
      routine_exercises: remapExerciseReferenceRows(envelope.tables.routine_exercises, exerciseIdResolution),
      pr_records: remapExerciseReferenceRows(envelope.tables.pr_records, exerciseIdResolution),
    },
  };
};

const applyBackupRestoreEnvelope = (envelope: BackupEnvelopeV1, fileName: string): ImportJobResult => {
  const sanitizedEnvelope = sanitizeBackupEnvelope(envelope);
  ...
  runInTransaction(() => {
    ...
    BACKUP_TABLES.forEach((tableName) => {
      const rows = sanitizedEnvelope.tables[tableName] ?? [];
      rows.forEach((row) => {
  ...
  refreshAnalyticsCaches();
  if (readAutoBackupStatus().enabled) {
    writeAutoBackupSnapshotSync();
  }
  ...
  writeAuditLog('backup_restore', fileName, 'restored', {
    version: sanitizedEnvelope.version,
    exportedAt: sanitizedEnvelope.exportedAt,
  });
...
const restoreBackupContent = (content: string, fileName: string): ImportJobResult => {
  const parsed = parseBackupEnvelope(content);
  const checksum = simpleChecksum(content);
  if (hasBackupExercises(parsed)) {
    return createBackupRestoreReview(parsed, fileName, checksum);
  }

  return applyBackupRestoreEnvelope(parsed, fileName);
};
...
export const decodeUtf8TextForTests = decodeUtf8Text;
export const repairUtf8MojibakeForTests = repairUtf8Mojibake;

export const restoreBackupTextForTests = (content: string, fileName = 'frog-backup-v1.json') =>
  restoreBackupContent(content, fileName);

export const createBackupEnvelopeForTests = (): BackupEnvelopeV1 => createBackupEnvelope(BACKUP_TABLES);
