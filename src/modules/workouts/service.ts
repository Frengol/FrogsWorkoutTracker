...
export type RoutineUpdateSuggestion = {
  routineId: string;
  routineName: string;
  changedExercisesCount: number;
};

type RoutineDetails = NonNullable<ReturnType<typeof getRoutineDetails>>;
    ...
    `
      SELECT
        ...
        COALESCE((
          ...
          JOIN set_entries se_cardio ON se_cardio.workout_exercise_id = we_cardio.id AND se_cardio.deleted_at IS NULL AND se_cardio.is_completed = 1
...
type LiveSetEntryField = (typeof liveSetEntryFields)[number];

const touchDraftSnapshotForSetEntry = (setId: string) => {
  database.runSync(
    `
      UPDATE workout_draft_snapshots SET updated_at = ?
      WHERE workout_id IN (
...
const touchDraftSnapshotForWorkoutExercise = (workoutExerciseId: string) => {
  database.runSync(
    `
      UPDATE workout_draft_snapshots SET updated_at = ?
      WHERE workout_id IN (
...
const touchDraftSnapshotForWorkout = (workoutId: string) => {
  database.runSync('UPDATE workout_draft_snapshots SET updated_at = ? WHERE workout_id = ?', nowIso(), workoutId);
};

export const updateSetEntryFields = ({
  setId,
  values,
}: {
  setId: string;
  values: Partial<Record<LiveSetEntryField, number | string | null>>;
}) => {
  const entries = liveSetEntryFields
    .filter((field) => Object.prototype.hasOwnProperty.call(values, field))
    .map((field) => [field, values[field] ?? null] as const);
  ...
  const fieldValues = entries.map(([, value]) => value);

  database.execSync('BEGIN');

  try {
    database.runSync(
      `UPDATE set_entries SET ${assignments}, updated_at = ? WHERE id = ?`,
      ...fieldValues,
      updatedAt,
      setId,
    );
    touchDraftSnapshotForSetEntry(setId);
    database.execSync('COMMIT');
  } catch (error) {
    database.execSync('ROLLBACK');
    throw error;
...
export const updateSetEntry = ({
  ...
  setId: string;
  field: LiveSetEntryField;
  value: number | string | null;
}) => updateSetEntryFields({ setId, values: { [field]: value } });
...
export const updateWorkoutExerciseNote = (workoutExerciseId: string, note: string) => {
  ...
  try {
    ...
    touchDraftSnapshotForWorkoutExercise(workoutExerciseId);
    database.execSync('COMMIT');
  } catch (error) {
    database.execSync('ROLLBACK');
    throw error;
...
export const updateWorkoutNote = (workoutId: string, note: string) => {
  ...
  try {
    ...
    touchDraftSnapshotForWorkout(workoutId);
    database.execSync('COMMIT');
  } catch (error) {
    database.execSync('ROLLBACK');
    throw error;
...
const getWorkoutRoutineStructure = (exercise: WorkoutLiveModel['exercises'][number]) => {
...
const countRoutineStructuralChanges = (
  routineExercises: RoutineDetails['exercises'],
  workoutExercises: WorkoutLiveModel['exercises'],
) => {
  const maxLength = Math.max(routineExercises.length, workoutExercises.length);
  let changedExercisesCount = 0;

  for (let index = 0; index < maxLength; index += 1) {
    const routineExercise = routineExercises[index];
    const workoutExercise = workoutExercises[index];

    if (!routineExercise || !workoutExercise) {
      changedExercisesCount += 1;
      continue;
    }

    const workoutStructure = getWorkoutRoutineStructure(workoutExercise);
    const routineWarmupEnabled = routineExercise.warmup_enabled === 1;

    if (
      routineExercise.exercise_id !== workoutStructure.exerciseId ||
      routineExercise.target_sets !== workoutStructure.targetSets ||
      routineWarmupEnabled !== workoutStructure.warmupEnabled
    ) {
      changedExercisesCount += 1;
    }
  }

  return changedExercisesCount;
};

const buildRoutineExerciseInputFromWorkout = (exercise: WorkoutLiveModel['exercises'][number]) => {
  const firstSet = exercise.sets[0];
  const cardioExercise = isCardioExercise(exercise.exercise);
  const workoutStructure = getWorkoutRoutineStructure(exercise);

  return {
    exerciseId: exercise.exercise.id,
    targetSets: workoutStructure.targetSets,
    ...
    warmupEnabled: workoutStructure.warmupEnabled,
  };
};

const buildRoutineComposerInputFromWorkout = (
  model: WorkoutLiveModel,
  routineMetadata: Pick<RoutineComposerInput, 'name' | 'description' | 'folderName'>,
): RoutineComposerInput => ({
  name: routineMetadata.name,
  description: routineMetadata.description,
  folderName: routineMetadata.folderName,
  exercises: model.exercises.map(buildRoutineExerciseInputFromWorkout),
});

const buildRoutineComposerInputFromQuickWorkout = (model: WorkoutLiveModel, routineName: string): RoutineComposerInput =>
  buildRoutineComposerInputFromWorkout(model, {
    name: routineName,
    description: '',
    folderName: '',
  });

export const getRoutineUpdateSuggestionForWorkout = (workoutId: string): RoutineUpdateSuggestion | null => {
  const model = getWorkoutLiveModel(workoutId);
  if (!model || model.workout.source !== 'routine' || !model.workout.routineId) {
    return null;
  }

  const routineDetails = getRoutineDetails(model.workout.routineId);
  if (!routineDetails) {
    return null;
  }

  const changedExercisesCount = countRoutineStructuralChanges(routineDetails.exercises, model.exercises);
  if (changedExercisesCount === 0) {
    return null;
  }

  return {
    routineId: model.workout.routineId,
    routineName: routineDetails.routine.name,
    changedExercisesCount,
  };
};

export const updateRoutineFromWorkout = (workoutId: string) => {
  const model = getWorkoutLiveModel(workoutId);
  if (!model || model.workout.source !== 'routine' || !model.workout.routineId) {
    return null;
  }
  ...
  const routineId = saveRoutine(
    buildRoutineComposerInputFromWorkout(model, {
      ...
      description: routineDetails.routine.description ?? '',
      folderName: routineDetails.routine.folder_name ?? '',
    }),
    model.workout.routineId,
  );

  writeAuditLog('workout', workoutId, 'routine_updated_from_workout', {
    routineId,
    changedExercisesCount,
  });
...
export const listCompletedWorkoutHistoryIds = ({
  dateFrom,
  dateTo,
}: {
