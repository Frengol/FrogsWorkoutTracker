...
import { exerciseCatalog } from '@/src/shared/content/library-content';
...
const createTables = () => {
  db.execSync(`
    ...
    CREATE TABLE IF NOT EXISTS user_preferences (
      ...
      auto_backup_last_exported_at TEXT
    );
...
const rebuildUserPreferencesWithoutNoOpFlags = () => {
  db.execSync(`
    CREATE TABLE user_preferences_v10 (
      ${baseColumnsSql},
      user_id TEXT NOT NULL,
      default_rest_seconds INTEGER NOT NULL,
      keep_awake INTEGER NOT NULL,
  ...
  db.execSync(`
    INSERT INTO user_preferences_v10 (
      id, created_at, updated_at, deleted_at, version, schema_version, remote_id, sync_state, last_exported_at, origin_device_id,
      user_id, default_rest_seconds, keep_awake, rest_overlay_enabled, week_starts_on
    )
    SELECT
      id, created_at, updated_at, deleted_at, version, schema_version, remote_id, sync_state, last_exported_at, origin_device_id,
      user_id, default_rest_seconds, keep_awake, rest_overlay_enabled, week_starts_on
  ...
  db.execSync('DROP TABLE user_preferences');
  db.execSync('ALTER TABLE user_preferences_v10 RENAME TO user_preferences');
};
...
const migrateToV10 = () => {
  ...
  try {
    if (
      tableHasColumn('user_preferences', 'haptics_enabled') ||
      tableHasColumn('user_preferences', 'show_rpe') ||
      tableHasColumn('user_preferences', 'show_previous_values')
    ) {
      rebuildUserPreferencesWithoutNoOpFlags();
    }
  } finally {
    db.execSync('PRAGMA foreign_keys = ON');
  }
...
const migrateToV11 = () => {
  if (!tableHasColumn('user_preferences', 'auto_backup_enabled')) {
    db.execSync('ALTER TABLE user_preferences ADD COLUMN auto_backup_enabled INTEGER NOT NULL DEFAULT 0');
  }

  if (!tableHasColumn('user_preferences', 'auto_backup_last_exported_at')) {
    db.execSync('ALTER TABLE user_preferences ADD COLUMN auto_backup_last_exported_at TEXT');
  }
};

const rebuildExercisesWithoutArchive = () => {
  db.execSync(`
    CREATE TABLE exercises_v12 (
      ${baseColumnsSql},
      slug TEXT NOT NULL UNIQUE,
      name TEXT NOT NULL,
      muscle_group TEXT NOT NULL,
  ...
  db.execSync('DROP TABLE exercises');
  db.execSync('ALTER TABLE exercises_v12 RENAME TO exercises');
};
...
const migrateToV12 = () => {
  ...
  try {
    rebuildExercisesWithoutArchive();
  } finally {
    db.execSync('PRAGMA foreign_keys = ON');
  }
};
  ...
  db.runSync(
    `
      INSERT INTO user_preferences (
        id, created_at, updated_at, deleted_at, version, schema_version, remote_id, sync_state, last_exported_at, origin_device_id,
        user_id, default_rest_seconds, keep_awake, rest_overlay_enabled, week_starts_on, auto_backup_enabled, auto_backup_last_exported_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `,
    preferences.id,
...
export const getUserPreferences = () =>
  database.getFirstSync<{
    ...
    auto_backup_last_exported_at: string | null;
