...
import { AnalyticsPeriod, DashboardView, ExerciseAnalyticsSnapshot, RecordMetric } from '@/src/shared/types/domain';
...
const isCardioAnalytics = (exercise: Pick<ExerciseAnalyticsSnapshot, 'muscleGroup'>) => exercise.muscleGroup === 'cardio';

const formatPace = (value?: number | null) => (value && value > 0 ? `${value.toFixed(1)} m/min` : '--');

const getDisplayRecordEntries = (exercise: ExerciseAnalyticsSnapshot) => {
  const allowedMetrics = isCardioAnalytics(exercise) ? cardioRecordMetrics : strengthRecordMetrics;
  return Object.entries(exercise.records).filter(([metric]) => allowedMetrics.has(metric as RecordMetric));
};
...
export default function ProgressScreen() {
  ...
  const exercisePickerKeyboardHeight = useKeyboardHeight(isExercisePickerVisible);
  const exercisePickerKeyboardStyles = getKeyboardAwareBottomSheetStyles({
    keyboardHeight: exercisePickerKeyboardHeight,
    viewportHeight,
  ...
  return (
    <AppScreen scroll keyboardAware measuredFocusScreenName="progress" testID="screen-progress">
      <ScreenHeader
        eyebrow="Progresso"
      ...
      <Modal
        ...
        <Pressable
          ...
          <Pressable
            style={[styles.modalCard, exercisePickerKeyboardStyles.cardStyle]}
            onPress={() => undefined}
...
const ExerciseAnalyticsCard = ({ exercise, onPress }: { exercise: ExerciseAnalyticsSnapshot; onPress: () => void }) => {
  const cardioExercise = isCardioAnalytics(exercise);
  const recordEntries = getDisplayRecordEntries(exercise);
    ...
    <Pressable onPress={onPress} testID={`card-progress-exercise-${exercise.exerciseId}`}>
      <Card>
        <View style={styles.exerciseHeader}>
          ...
          <Text style={styles.listValue}>{cardioExercise ? `${exercise.sessions} sessões` : formatWeight(exercise.bestWeight)}</Text>
        </View>

        <View style={styles.grid}>
          {cardioExercise ? (
            <>
              <MetricTile label="Sessões" value={String(exercise.sessions)} />
              ...
              <MetricTile label="Melhor ritmo" value={formatPace(exercise.bestPaceMetersPerMinute)} />
            </>
          ) : (
            <>
              <MetricTile label="Melhor carga" value={formatWeight(exercise.bestWeight)} />
              <MetricTile label="1RM" value={formatWeight(exercise.bestEstimated1Rm)} />
              <MetricTile label="Melhor série" value={formatWeight(exercise.bestSetVolume)} />
              <MetricTile label="Melhor sessão" value={formatWeight(exercise.bestSessionVolume)} />
          ...
          <>
            <LineTrendChart
              data={[...exercise.history]
                .reverse()
                .map((entry) => ({
                  x: getShortDateLabel(entry.dayKey),
                  y: Math.round(cardioExercise ? entry.totalDistanceMeters || entry.totalDurationSeconds : entry.totalVolume),
                }))}
            />
            <View style={styles.historyList}>
              {exercise.history.map((entry) => (
                <View key={`${exercise.exerciseId}-${entry.dayKey}`} style={styles.historyRow}>
                  ...
                  {cardioExercise ? (
                    <>
                      <Text style={styles.historyText}>{entry.totalDurationSeconds ? formatDuration(entry.totalDurationSeconds) : '--'}</Text>
                      <Text style={styles.historyText}>{formatDistance(entry.totalDistanceMeters)}</Text>
                      <Text style={styles.historyText}>{formatPace(entry.bestPaceMetersPerMinute)}</Text>
                    </>
                  ) : (
                    <>
