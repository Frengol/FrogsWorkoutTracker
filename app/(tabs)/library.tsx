...
import { consumeLibrarySuccessNotice } from '@/src/shared/config/library-success-notice';
import { routes } from '@/src/shared/navigation/routes';
import { Equipment, Exercise, MuscleGroup } from '@/src/shared/types/domain';
...
export default function LibraryScreen() {
  ...
  const [, refreshExercises] = useState(0);
  ...
  useFocusEffect(
    useCallback(() => {
      reload();
      const nextNotice = consumeLibrarySuccessNotice();
      if (nextNotice) {
        setRoutineTransferFeedback(nextNotice);
      }
    }, [reload]),
  );
  ...
  const filteredExercises = listExercises({
    search,
  ...
  }, [exerciseSource, mode, search, selectedEquipment, selectedMuscle]);
  ...
  const handleImportRoutineJson = async () => {
    ...
    try {
      ...
      if (result.status === 'pending_review' && result.reviewJobId) {
        setRoutineTransferFeedback('Importação pronta para revisar exercícios importados.');
  ...
  const handleDeleteCustomExercise = async (exercise: Exercise) => {
    ...
    const confirmed = await dialog.confirm({
      ...
      message: `Deseja excluir "${exercise.name}"? Se ele já foi usado em treinos, rotinas ou recordes, o Frogs mantém o histórico e remove apenas da lista ativa.`,
      confirmLabel: 'Excluir',
      tone: 'danger',
    });
    ...
    try {
      ...
      setRoutineTransferFeedback('Exercício personalizado excluído.');
      refreshExercises((current) => current + 1);
    } catch (error) {
      setRoutineTransferFeedback(error instanceof Error ? error.message : 'Não foi possível excluir o exercício.');
    }
  };
  ...
  const handleLoadMoreExercises = useCallback(() => {
    setVisibleExerciseCount((current) => Math.min(current + EXERCISE_PAGE_SIZE, filteredExercises.length));
  }, [filteredExercises.length]);

  const handleExerciseListScroll = useCallback(
    (event: NativeSyntheticEvent<NativeScrollEvent>) => {
      const { contentOffset, contentSize, layoutMeasurement } = event.nativeEvent;
      const distanceFromEnd = contentSize.height - (contentOffset.y + layoutMeasurement.height);

      if (distanceFromEnd <= 80) {
        handleLoadMoreExercises();
      }
    },
    [handleLoadMoreExercises],
  );
  ...
  const renderExerciseItem: ListRenderItem<Exercise> = ({ item: exercise }) => (
    <Pressable
      key={exercise.id}
      onPress={() => router.push(routes.exercises.detail(exercise.id))}
      onLongPress={() => {
        if (exercise.isCustom) {
          handleDeleteCustomExercise(exercise).catch(() => undefined);
        }
      }}
      testID={`item-library-exercise-${exercise.id}`}>
  ...
  return (
    <>
      {mode === 'exercises' ? (
        <AppScreen noBottomPadding testID="screen-library">
          <FlatList
            ...
            contentContainerStyle={[styles.exerciseListContent, { paddingBottom: spacing.xxxl + insets.bottom }]}
            initialNumToRender={visibleExercises.length}
            keyboardShouldPersistTaps="handled"
            maxToRenderPerBatch={visibleExercises.length}
            onEndReached={handleLoadMoreExercises}
            onEndReachedThreshold={0.35}
            onScroll={handleExerciseListScroll}
            scrollEventThrottle={16}
            testID="list-library-exercises"
          />
        </AppScreen>
      ) : (
        <AppScreen scroll keyboardAware measuredFocusScreenName="library" testID="screen-library">
          {commonHeader}
          <>
