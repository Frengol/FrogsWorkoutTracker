...
import { consumeProfileSuccessNotice } from '@/src/shared/config/profile-success-notice';
...
import { AppScreen, Card, EmptyState, Field, HeaderIconButton, PrimaryButton, ScreenHeader, SecondaryButton, SectionTitle } from '@/src/shared/design/ui';
...
const HISTORY_MENU_HEIGHT_WITH_ROUTINE = 192;
const HISTORY_MENU_BUTTON_SIZE = 34;
const voluntaryContributionImage = require('../../assets/images/donate.png');
...
export default function ProfileScreen() {
  ...
  const [createRoutineModal, setCreateRoutineModal] = useState<CreateRoutineModalState | null>(null);
  ...
  const [profileSuccessNotice, setProfileSuccessNotice] = useState('');
  const historyListBottomPadding = spacing.xxl + Math.max(insets.bottom, spacing.md) + 64;
  const createRoutineKeyboardHeight = useKeyboardHeight(createRoutineModal != null);
  const createRoutineKeyboardStyles = getKeyboardAwareBottomSheetStyles({
    keyboardHeight: createRoutineKeyboardHeight,
    viewportHeight,
  ...
  const historyMenuFrame = useMemo(() => {
    ...
    const menuHeight = openHistoryMenu.source === 'empty' ? HISTORY_MENU_HEIGHT_WITH_ROUTINE : HISTORY_MENU_HEIGHT;
  ...
  useFocusEffect(
    useCallback(() => {
      refreshHistory();
      const nextNotice = consumeProfileSuccessNotice();
      if (nextNotice) {
        setProfileSuccessNotice(nextNotice);
      }
    }, [refreshHistory]),
  );
  ...
  useEffect(() => {
    if (!profileSuccessNotice) {
      return undefined;
    }

    const timeoutId = setTimeout(() => {
      setProfileSuccessNotice('');
    }, 10000);
  ...
  const handleImportWorkoutCsv = async () => {
    ...
    try {
      ...
      if (result.status === 'pending_review' && result.reviewJobId) {
        setHistoryImportFeedback('Importação pronta para revisar exercícios importados.');
  ...
  const handleOpenCreateRoutineModal = (workoutId: string, workoutTitle: string) => {
    setOpenHistoryMenu(null);
    setHistoryImportFeedback('');
    setCreateRoutineError('');
    setCreateRoutineModal({
      workoutId,
      workoutTitle,
      name: workoutTitle,
    });
  ...
  const handleCancelCreateRoutine = () => {
    setCreateRoutineModal(null);
    setCreateRoutineError('');
    setIsCreatingRoutine(false);
  };

  const handleConfirmCreateRoutine = () => {
    if (!createRoutineModal) {
      return;
    }

    const trimmedName = createRoutineModal.name.trim();
    if (!trimmedName) {
      setCreateRoutineError('Informe um nome para criar a rotina.');
      return;
    }

    try {
      setIsCreatingRoutine(true);
      const routineId = saveQuickWorkoutAsRoutine(createRoutineModal.workoutId, trimmedName);
      if (!routineId) {
      ...
      setCreateRoutineModal(null);
      setCreateRoutineError('');
      setHistoryImportFeedback('Rotina criada na Biblioteca.');
    } catch (error) {
      setCreateRoutineError(error instanceof Error ? error.message : 'Não foi possível criar a rotina.');
    } finally {
      setIsCreatingRoutine(false);
    }
  };

  const handleOpenVoluntaryContribution = async () => {
    setOpenHistoryMenu(null);

    try {
      await Linking.openURL(VOLUNTARY_CONTRIBUTION_URL);
    } catch {
      await dialog.alert({
  ...
  const handleOpenHistoryMenu = useCallback(
    (workoutId: string, title: string, source: string, event: GestureResponderEvent) => {
  ...
  const header = (
    <View style={styles.headerContent}>
      ...
      <Card style={styles.contributionCard} testID="card-profile-voluntary-contribution">
        <View style={styles.contributionTopRow}>
          <Image
            accessibilityLabel="Imagem de apoio ao projeto"
            accessible
            resizeMode="contain"
            source={voluntaryContributionImage}
            style={styles.contributionImage}
          />
          <View style={styles.contributionBody}>
            <Text style={styles.contributionTitle}>Apoie o Frogs</Text>
        ...
        <View style={styles.contributionFooter}>
          <PrimaryButton
            label="Apoiar via Mercado Pago"
            onPress={() => {
              handleOpenVoluntaryContribution().catch(() => undefined);
            }}
            style={styles.contributionButton}
            testID="btn-profile-voluntary-contribution"
          />
          <Text style={styles.contributionHint}>Escolha qualquer valor no Mercado Pago.</Text>
        </View>
      </Card>
      ...
      {profileSuccessNotice ? (
        <Card testID="card-profile-success-notice">
          <Text style={styles.historyImportFeedback}>{profileSuccessNotice}</Text>
        </Card>
      ) : null}
  ...
  return (
    <AppScreen noBottomPadding style={styles.screen} testID="screen-profile">
      <FlatList
                ...
                <View style={styles.historyCardHeader}>
                  <View style={styles.historyTitleRow} testID={`row-profile-history-title-${item.id}`}>
                    ...
                    <Pressable
                      ...
                      onPress={(event) => handleOpenHistoryMenu(item.id, workoutTitle, item.source, event)}
                      ref={(node) => {
        ...
        <Pressable
          ...
          {openHistoryMenu && historyMenuFrame ? (
            <View style={[styles.historyMenu, { left: historyMenuFrame.left, top: historyMenuFrame.top }]}>
              ...
              {openHistoryMenu.source === 'empty' ? (
                <Pressable
                  accessibilityRole="button"
                  accessibilityLabel={`Criar rotina a partir do treino ${openHistoryMenu.title}`}
                  onPress={() => {
                    handleOpenCreateRoutineModal(openHistoryMenu.workoutId, openHistoryMenu.title);
                  }}
                  style={styles.historyMenuItem}
                  testID={`btn-profile-history-create-routine-${openHistoryMenu.workoutId}`}>
                  <Text style={styles.historyMenuItemText}>Criar rotina</Text>
                </Pressable>
              ) : null}
              <Pressable
                accessibilityRole="button"
      ...
      <Modal
        animationType="fade"
        onRequestClose={handleCancelCreateRoutine}
        transparent
        visible={createRoutineModal != null}
        testID="modal-profile-history-create-routine">
        <Pressable
          onPress={handleCancelCreateRoutine}
          style={[styles.createRoutineModalBackdrop, createRoutineKeyboardStyles.backdropStyle]}
          testID="backdrop-profile-history-create-routine">
          <Pressable
            onPress={() => undefined}
            style={[styles.createRoutineModalCard, createRoutineKeyboardStyles.cardStyle]}
            testID="card-profile-history-create-routine">
            <Text style={styles.createRoutineModalTitle}>Criar rotina</Text>
            <Field
              label="Nome da rotina"
              value={createRoutineModal?.name ?? ''}
              onChangeText={(value) =>
                setCreateRoutineModal((current) => (current ? { ...current, name: value } : current))
              }
              placeholder="Nome da rotina"
              testID="input-profile-history-create-routine-name"
            />
            {createRoutineError ? <Text style={styles.createRoutineModalError}>{createRoutineError}</Text> : null}
            <View style={styles.createRoutineModalActions}>
              <SecondaryButton
                label="Cancelar"
                onPress={handleCancelCreateRoutine}
                style={{ flex: 1 }}
                testID="btn-profile-history-create-routine-cancel"
              />
              <PrimaryButton
                label={isCreatingRoutine ? 'Criando...' : 'Confirmar'}
                onPress={handleConfirmCreateRoutine}
                disabled={isCreatingRoutine}
                style={{ flex: 1 }}
                testID="btn-profile-history-create-routine-confirm"
              />
            </View>
          </Pressable>
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  createRoutineModalBackdrop: {
    flex: 1,
    justifyContent: 'center',
    padding: spacing.lg,
    backgroundColor: colors.overlay,
  },
  createRoutineModalCard: {
    gap: spacing.md,
  ...
  createRoutineModalTitle: {
    fontFamily: typography.heading,
    fontSize: 20,
    color: colors.text,
  },
  createRoutineModalActions: {
    flexDirection: 'row',
    gap: spacing.md,
  },
  createRoutineModalError: {
    fontFamily: typography.bodySemi,
    fontSize: 13,
    color: colors.danger,
  },
