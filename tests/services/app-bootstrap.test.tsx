...
jest.mock('@/src/modules/data-transfer/service', () => ({
  syncAutoBackupSnapshotIfEnabled: jest.fn(async () => ({
    enabled: true,
...
import { AppState, AppStateStatus, Text, Pressable } from 'react-native';
...
describe('AppBootstrapProvider', () => {
  ...
  it('syncs the Android auto backup payload when the app goes to background', async () => {
    const screen = renderScreen(
      <AppBootstrapProvider>
        <Consumer />
      </AppBootstrapProvider>,
    ...
    await waitFor(() => expect(syncAutoBackupSnapshotIfEnabled).toHaveBeenCalledTimes(1));
  });
