...
describe('routines service', () => {
  ...
  it('does not create an empty folder when the folder name is only spaces', () => {
    const routineId = saveRoutine({
      name: 'Treino sem pasta',
      description: '',
      folderName: '   ',
      exercises: [],
    });

    expect(routineId).toBe('entity-1');
    expect(database.getFirstSync).not.toHaveBeenCalledWith(
      expect.stringContaining('routine_folders'),
      expect.any(String),
    );
    expect(
      (database.runSync as jest.Mock).mock.calls.some(([sql]) => String(sql).includes('INSERT INTO routine_folders')),
    ).toBe(false);
    expect(database.runSync).toHaveBeenCalledWith(
      expect.stringContaining('INSERT INTO routines'),
      'entity-1',
