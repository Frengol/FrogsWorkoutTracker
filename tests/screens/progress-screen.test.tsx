...
describe('ProgressScreen', () => {
  ...
  it('keeps body measurement inputs reachable with measured focus', () => {
    jest.useFakeTimers();
    ...
    act(() => {
      ...
      scrollView.props.onScroll({ nativeEvent: { contentOffset: { y: 420 } } });
    });
    fireEvent(screen.getByTestId('input-progress-measurement-note'), 'layout', {
      nativeEvent: { layout: { y: 1800, height: 96 } },
    });
    fireEvent(screen.getByTestId('input-progress-measurement-note'), 'focus');
    act(() => {
  ...
  it('renders a single selected exercise card with empty records/history fallbacks and opens the detail screen', () => {
    ...
    expect(screen.getByText('Total de reps')).toBeTruthy();
    expect(screen.getAllByText('560 kg').length).toBeGreaterThan(0);
    expect(screen.getByText('40 repetições')).toBeTruthy();
  ...
  it('renders exercise analytics with pace and duration data when available', () => {
    ...
    (listExerciseAnalytics as jest.Mock).mockReturnValue([
      createExerciseAnalyticsSnapshot({
        ...
        records: { best_duration: 95, best_distance: 400 },
        history: [
          {
            dayKey: '2026-03-25',
            totalVolume: 0,
    ...
    expect(screen.getByText('Melhor ritmo')).toBeTruthy();
    expect(screen.getAllByText('12.3 m/min').length).toBeGreaterThan(0);
    expect(screen.getAllByText('1m 35s').length).toBeGreaterThan(0);
    ...
    expect(screen.queryByText('Total de reps')).toBeNull();
