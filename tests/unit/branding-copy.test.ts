...
describe('branding copy', () => {
  ...
  it('preserves stable technical identifiers and example data', () => {
    ...
    expect(readRepoFile('src/shared/db/database.ts')).toContain('frog-workout-tracker.db');
    ...
    expect(readRepoFile('data/exercises.catalog.json')).toContain('"slug": "frog-pump"');
    expect(readRepoFile('data/exercises.catalog.json')).toContain('"Frog Pump"');
  });
});
