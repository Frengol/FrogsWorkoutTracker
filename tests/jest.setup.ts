...
jest.mock('react-native-draggable-flatlist', () => {
  ...
  const DraggableFlatList = React.forwardRef(({
    ...
    React.useImperativeHandle(ref, () => methodsRef.current);
    ...
    return React.createElement(
      View,
      {
        ...
        contentContainerStyle,
        scrollToEnd: methodsRef.current.scrollToEnd,
        scrollToIndex: methodsRef.current.scrollToIndex,
        scrollToOffset: methodsRef.current.scrollToOffset,
        keyboardDismissMode,
        keyboardShouldPersistTaps,
        onScroll,
        onScrollOffsetChange,
...
jest.mock('expo-notifications', () => {
  ...
  let shouldFailNextScheduledRead = false;
  let failedScheduleWeekdays = new Set<number>();
  ...
  const api = {
    ...
    scheduleNotificationAsync: jest.fn(async ({ content, trigger }) => {
      if (failedScheduleWeekdays.has(trigger?.weekday)) {
        throw new Error(`Falha ao agendar ${trigger.weekday}`);
      }
    ...
    getAllScheduledNotificationsAsync: jest.fn(async () => {
      if (shouldFailNextScheduledRead) {
        shouldFailNextScheduledRead = false;
        throw new Error('Fila nativa indisponível');
      }
    ...
    __resetMockNotifications: () => {
      ...
      shouldFailNextScheduledRead = false;
      failedScheduleWeekdays = new Set();
    },
    __setPermissionsGranted: (value: boolean) => {
      permissionsGranted = value;
    },
    __failNextScheduledRead: () => {
      shouldFailNextScheduledRead = true;
    },
    __failScheduleForWeekdays: (weekdays: number[]) => {
      failedScheduleWeekdays = new Set(weekdays);
    },
...
jest.mock('expo-file-system', () => {
  const normalizeUri = (value: string) => {
    if (value === 'file:' || value === 'file://' || value === 'file:///') {
      return 'file:///';
    }
  ...
  const textFailures = new Map<string, Error>();
  const byteFailures = new Map<string, Error>();
  ...
  class File {
    ...
    write(content: string) {
      ...
      textOverrides.delete(this.uri);
      textFailures.delete(this.uri);
      byteFailures.delete(this.uri);
    }

    async text() {
      const failure = textFailures.get(this.uri);
      if (failure) {
        throw failure;
      }
      return textOverrides.get(this.uri) ?? files.get(this.uri) ?? '';
    }

    async bytes() {
      const failure = byteFailures.get(this.uri);
      if (failure) {
        throw failure;
      }
    ...
    __setTextForTests(content: string) {
      textOverrides.set(this.uri, String(content));
    }

    __setTextFailureForTests(error: Error) {
      textFailures.set(this.uri, error);
    }

    __setBytesFailureForTests(error: Error) {
      byteFailures.set(this.uri, error);
    }

    __getContentForTests() {
    ...
    copy(target: { uri: string }) {
      ...
      const textOverride = textOverrides.get(this.uri);
      if (textOverride === undefined) {
        textOverrides.delete(target.uri);
      } else {
        textOverrides.set(target.uri, textOverride);
      }
      textFailures.delete(target.uri);
    }
  ...
  return {
    ...
    __writeMockFileForTests: (uri: string, content: string) => {
      files.set(normalizeUri(uri), String(content));
    },
